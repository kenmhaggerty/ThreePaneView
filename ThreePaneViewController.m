...
#pragma mark - // UINavigationItem (Overwrite) //

#pragma mark Category Interface

@interface UINavigationItem (Overwrite)
- (void)overwriteWithItem:(UINavigationItem *)item;
@end

#pragma mark Implementation

@implementation UINavigationItem (Overwrite)
...
@interface ThreePaneViewController () <UITextFieldDelegate>
...
@property (nonatomic, strong) IBOutlet NSLayoutConstraint *constraintOverlap;
...
- (IBAction)closeSideView:(id)sender;
...
- (void)setSideViewWidth:(CGFloat)sideViewWidth {
    [self setSideViewWidth:sideViewWidth animated:NO completion:nil];
}
...
- (void)setOverlap:(CGFloat)overlap {
    self.constraintOverlap.constant = overlap;
}

- (CGFloat)overlap {
    return self.constraintOverlap.constant;
}
